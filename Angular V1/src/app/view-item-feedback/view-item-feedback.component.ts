...
export class ViewItemFeedbackComponent implements OnInit {
  ...
  getItemFeedback(){
    this.service.listItemReviewById(this.item.itemId).subscribe(data => {this.feedback = data;
  ...
  getAvgRating(){
    this.service.searchItemAverageRating(this.item.itemId).subscribe(data=>{this.avg = data;
