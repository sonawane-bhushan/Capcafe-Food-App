...
export class ViewCafeFeedbackComponent implements OnInit {
  ...
  getCafeReview() {
    this.service.listCafeReviewById(this.cafe.cafeId).subscribe(data => {
      this.cafeReview = data;
  ...
  viewCafeAvgRating() {
    this.service.searchCafeAverageRating(this.cafe.cafeId).subscribe(data => {
      this.avgRating = data;
